...
const inputClasses = ["absolute", "overflow-hidden", "outline-none", "disabled:cursor-not-allowed", "border-0", "opacity-0", "z-10", "peer", "rounded-full"]
...
const spanClasses = ["leading-none", "bg-secondary-500", "dark:bg-secondary-800", "peer-disabled:bg-secondary-100",
  "peer-checked:peer-disabled:bg-success-100", "peer-checked:bg-success-500", "before:bg-white", "rounded-full",
  "before:rounded-full", "inline-flex", "relative", "before:absolute", "before:top-0", "before:bottom-0",
  "before:my-auto", "before:transition", "transition", "peer-focus-visible:ring-2", "before:translate-x-0.5"]

const inputSizeSM = ["w-[30px]", "h-4"]
const inputSizeMD = ["w-[38px]", "h-5"]
const inputSizeLG = ["w-[46px]", "h-6"]
const spanSizeSM = ["w-[30px]", "h-4", "before:w-3", "before:h-3", "peer-checked:before:translate-x-4"]
const spanSizeMD = ["w-[38px]", "h-5", "before:w-4", "before:h-4", "peer-checked:before:translate-x-5"]
const spanSizeLG = ["w-[46px]", "h-6", "before:w-5", "before:h-5", "peer-checked:before:translate-x-6"]
const stateNull = ["peer-focus-visible:ring-primary-500/40"]
const stateWarning = ["peer-focus-visible:ring-warning-500/40"]
const stateDanger = ["peer-focus-visible:ring-danger-500/40"]
const stateSuccess = ["peer-focus-visible:ring-success-500/40"]
...
describe("LuiSwitch.vue", () => {
  ...
  it("should render classes correctly", () => {
    ...
    expect(wrapper.find('span').classes().sort()).toEqual(spanClasses.concat(spanSizeMD, stateNull).sort())
  })
  it("sould render size classes correctly", () => {
    ...
    expect(wrapper.find('span').classes().sort()).toEqual(spanClasses.concat(spanSizeSM, stateNull).sort())
  })
})