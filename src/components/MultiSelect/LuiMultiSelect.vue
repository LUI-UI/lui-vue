...
const { properPosition } = useFindProperPosition(wrapperRef)
...
const optionWrapperClasses = computed(() => {
  const optionsWrapper: TwClassInterface = {
    ...
    space: props.size === 'xs' || props.size === 'sm' ? 'space-y-px' : 'space-y-0.5'
  }
...
function updateSelectedOptions(option: ModelValue, isInitial: boolean = false) {
  ...
  if (!isInitial) emit('change', selectedOptions.value)
  if (props.searchable) {
    searchQuery.value = ''
...
function dynamicSelectionAttributes(option: any) {
  return props.tags
    ? {
        ...
        filter: props.tagFilter,
        class: 'mx-px'
      }
    : {}
}

const LuiText = (option: any, index: number) =>
  h(
    'span', // type
...
<template>
  <div
    class="relative"
    :class="[block ? '' : 'max-w-[400px]', props.disabled ? 'cursor-not-allowed' : '']"
  >
    <div
      ref="wrapperRef"
      role="combobox"
