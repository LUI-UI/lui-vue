...
export function useButtonClasses(props: PropTypes) {
  ...
  const computedButtonClasses = computed(() => {
    const buttonClasses: TwClassInterface = {
      ...
      // 4 4 6 8 8
      space: classNames(props.prepend.value !== "none" || props.append.value !== "none" ? {
  ...
  const computedIconSize = computed(() =>
    // 12 - 16- 20 - 20 - 24
    props.size.value === "xs" ? "text-xs" : props.size.value === "sm" ? "text-base" : props.size.value === "xl" ? "text-2xl" : "text-xl"
